...
- [x] Глубокая миграция inline цветов/стилей на тему: **174 → 73** hardcoded `Colors.*` (-101). Переведены: `modal`, `square_button`, `app_bar`, `main` status bar, `qa_actions`, `login`, `tasks_filter_sheet`, `equipment_list/detail_screen`, `notifications_screen`+`notification_card`, `qr_result_screen` (passport + image), `result_controls`, `qr_screen`, `qr_inventory_result_screen`, `onboarding_*`, `notifications_settings_screen`, `help_link`, `select_*` family. Использованы: `colorScheme.surface/inverseSurface/primary/onPrimary/onSurface/onSurfaceVariant/outline/outlineVariant/tertiary/error`, `textTheme.*`, `AppConstants.radiusSM/MD/XL` + `spacingSM/MD/LG`. Остаток 73 — **семантические** (periodColors, priority red/orange/green, SnackBar accent, индикаторы статуса сервиса) — менять = потерять смысл
