...
Future<void> main() async {
  ...
  // То же самое для наработки — и по гораздо более серьёзному поводу.
  //
  // У осмотров запись, застрявшая в pending, вернулась бы и без этого цикла:
  // у каждой попытки есть отметка времени (`scan_pending_<key>`), по которой
  // она возвращается через 120 секунд. У наработки такой отметки нет, а
  // `syncUsageScans` перебирает только `scanUsageBox` — значит запись,
  // оставшаяся в pending после гибели процесса во время `PATCH`, не уйдёт
  // никогда.
  //
  // Стоило бы это не самой наработке, а закрытию ТО: его шлюз ждёт, пока
  // уедет наработка по тому же оборудованию, и проверяет pending-бокс **без**
  // фильтра по отказу — пометке там взяться неоткуда. Осмотр задачи ТО не
  // ушёл бы никогда, ЗИП не списался бы никогда, и на экране не появилось бы
  // ни слова: наработка не помечена отклонённой, полоса на главной молчит.
  // Заодно `_outboxIsEmpty` навсегда оставался бы `false`, и ранний выход
  // цикла отправки переставал работать.
  //
  // Повтор безвреден: если предыдущий запрос всё-таки дошёл, сервер примет
  // тот же `current_value` ещё раз.
  //
  // Кладём в основной бокс до удаления из pending, а не после: оборвись
  // процесс посередине, лишняя копия лучше потерянного показания. Снимок
  // списка (`toList`) — потому что бокс правится прямо в цикле.
  for (final usage in dataProvider.scanUsagePendingBox.values.toList()) {
    await dataProvider.scanUsageBox.put(usage.key(), usage);
    await dataProvider.scanUsagePendingBox.delete(usage.key());
  }
