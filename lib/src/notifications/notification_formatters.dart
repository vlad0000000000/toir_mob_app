...
class NotificationFormatters {
  ...
  ///
  /// Та же история с ремонтами: `repair_assigned` приходит и на «вам назначен»,
  /// и на «свободный, можно взять в работу» — тип один, различается только
  /// заголовок. Без отдельной метки чип «Назначен ремонт» противоречил бы
  /// заголовку свободного ремонта.
  static String typeLabel(AppNotification n) {
    if (n.status == NotificationStatuses.unassigned) {
      return 'Снятие с осмотра';
    }
    if (n.isFreeRepair) {
      return 'Свободный ремонт';
    }
