...
class RepairCardStrings {
  ...
  /// Подробности ремонта в этот заход не загрузились, и что записано в
  /// расходе — неизвестно.
  ///
  /// Отдельно от [consumptionEmptyNote] и ровно ради честности: список
  /// ремонтов расход не отдаёт, поэтому пустой блок означал «не знаю», а
  /// подпись утверждала «расхода нет». У ремонта «На рассмотрении» блок ещё и
  /// нередактируем, так что поправить эту «пустоту» обходчик не мог — и
  /// оставался уверен, что расход потерян.
  static const String consumptionUnknownNote =
      'Расход не загружен — нет связи с сервером. Показать его нечем: '
      'подключитесь и потяните карточку вниз.';
